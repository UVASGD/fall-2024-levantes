...
@onready var y_axis_head = %y_axis_head
...
var projectile = preload("res://projectiles/enemy_projectile_shotgun.tscn")
...
func face_target(delta):
	...
	if can_move_y_axis:
		...
		y_axis_head.current_turn_speed = y_axis_head.normal_turn_speed
		y_axis.face_point(target_pos, delta)
		y_axis_head.face_point(target_pos, delta)

func update_turn_speed(new_speed):
	x_axis.current_turn_speed = new_speed
	y_axis.current_turn_speed = new_speed
	y_axis_head.current_turn_speed = new_speed
...
func spawn_projectile():
	...
	for x in range(grid_size):
		for y in range(grid_size):
			...
			# Calculate the direction with grid offset
			var direction = (target_pos - spawn_pos).normalized()
			direction += Vector3((x - half_grid_size) * grid_spacing, 
									(y - half_grid_size) * grid_spacing, 0).normalized() * 0.2

			# Set projectile's rotation and velocity
			projectile_instance.look_at(projectile_instance.global_transform.origin + direction)
			projectile_instance.velocity = direction * projectile_speed
...
func take_damage(amount: int):
	health_hp -= amount
	if health_hp < max_health and !is_helmet_broken:
		%Helmet.hide()
		%BrokenHelmet.show()
...
func on_hit(damage_taken, hs_mult, col, shape):
	...
	if not is_dying and col==collider and (shape == 1 or shape == 0):
		...
		#x_axis_shield.show()
		#y_axis_shield.show()
		$hit_death_anim_player.play("hit")
...
func death():
	...
	spawn_reward()
	next_state = "idle"
	$hit_death_anim_player.play("death")
	await $hit_death_anim_player.animation_finished
	$".".queue_free()
